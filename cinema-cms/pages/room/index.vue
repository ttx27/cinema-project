<template>
  <div class="agency-table-groups">
    ...
    <div class="text-right">
      <a-button type="primary" class="editable-add-btn mb-3" @click="showModalAdd">Thêm phòng mới</a-button>
    </div>
    <a-table
      size="middle"
      :columns="columns"
    ...
    <a-modal
      ...
      @cancel="hideModalAdd"
    >
      <RoomsFormAdd v-if="modalAddShow" @submitted="hideModalAdd" />
    </a-modal>
    ...
    >
      <RoomsFormEdit v-if="modalEditShow" @submitted="hideModalEdit" :initialValues="editData" />
    </a-modal>
  </div>
</template>
...
import RoomsFormAdd from '~/components/Rooms/RoomsFormAdd';
import RoomsFormEdit from '~/components/Rooms/RoomsFormEdit';
...
export default {
  ...
  methods: {
    ...
    async handleDelete(id) {
      ...
      try {
        const doDelete = await this.$store.dispatch('rooms/delete', id);
        if (!doDelete.success && doDelete.err_msg) {
          this.$message.error(doDelete.err_msg);
        } else {
          this.$message.success('Xóa phòng thành công!');
          await this.reloadTableData();
        }
      } catch (error) {
          this.$message.error('Xóa phòng không thành công');
      }
