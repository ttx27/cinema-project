<template>
  <div class="agency-table-groups">
    ...
    <div class="text-right">
      <a-button type="primary" class="editable-add-btn mb-3" @click="showModalAdd">Thêm ghế mới</a-button>
    </div>
    <a-table
      size="middle"
      :columns="columns"
    ...
    <a-modal
      ...
      @cancel="hideModalAdd"
    >
      <SeatsFormAdd v-if="modalAddShow" @submitted="hideModalAdd" />
    </a-modal>

    <a-modal
      :maskClosable="false"
      v-if="modalEditShow"
      title="Sửa thông tin ghế"
    ...
    >
      <SeatsFormEdit v-if="modalEditShow" @submitted="hideModalEdit" :initialValues="editData" />
    </a-modal>
  </div>
</template>
...
import SeatsFormAdd from '~/components/Seats/SeatsFormAdd';
import SeatsFormEdit from '~/components/Seats/SeatsFormEdit';
...
export default {
  ...
  async fetch({ store }) {
    await Promise.all([
      ...
      store.dispatch('seatTypes/getOptions'),
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          title: 'Loại ghế',
          dataIndex: 'seatType.name'
        },
        {
          title: 'Giá',
          dataIndex: 'seatType.price',
  ...
  methods: {
    ...
    async handleDelete(id) {
      ...
      try {
        const doDelete = await this.$store.dispatch('seats/delete', id);
        if (!doDelete.success && doDelete.err_msg) {
          this.$message.error(doDelete.err_msg);
        } else {
          this.$message.success('Xóa ghế thành công!');
          await this.reloadTableData();
        }
      } catch (error) {
          this.$message.error('Xóa ghế không thành công');
      }
    ...
    async reloadTableData(reloadOption) {
      ...
      await this.$store.dispatch('seats/get', {});
