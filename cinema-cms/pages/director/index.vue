<template>
  <div class="agency-table-groups">
    ...
    <div class="text-right">
      <a-button type="primary" class="editable-add-btn mb-3" @click="showModalAdd">Thêm đạo diễn mới</a-button>
    </div>
    <a-table
      size="middle"
      :columns="columns"
    ...
    <a-modal
      ...
      @cancel="hideModalAdd"
    >
      <DirectorsFormAdd v-if="modalAddShow" @submitted="hideModalAdd" />
    </a-modal>

    <a-modal
      :maskClosable="false"
      v-if="modalEditShow"
      title="Sửa thông tin đạo diễn"
    ...
    >
      <DirectorsFormEdit v-if="modalEditShow" @submitted="hideModalEdit" :initialValues="editData" />
    </a-modal>
  </div>
</template>
...
import DirectorsFormAdd from '~/components/Directors/DirectorsFormAdd';
import DirectorsFormEdit from '~/components/Directors/DirectorsFormEdit';
...
export default {
  ...
  methods: {
    ...
    async handleDelete(id) {
      ...
      try {
        const doDelete = await this.$store.dispatch('directors/delete', id);
        if (!doDelete.success && doDelete.err_msg) {
          this.$message.error(doDelete.err_msg);
        } else {
          this.$message.success('Xóa đạo diễn thành công!');
          await this.reloadTableData();
        }
      } catch (error) {
          this.$message.error('Xóa đạo diễn không thành công');
      }
    ...
    async reloadTableData(reloadOption) {
      ...
      await this.$store.dispatch('directors/get', {});
