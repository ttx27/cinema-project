<template>
  <div class="agency-table-groups">
    ...
    <div class="text-right">
      <a-button type="primary" class="editable-add-btn mb-3" @click="showModalAdd">Thêm ngôn ngữ mới</a-button>
    </div>
    <a-table
      size="middle"
      :columns="columns"
    ...
    <a-modal
      ...
      @cancel="hideModalAdd"
    >
      <LanguagesFormAdd v-if="modalAddShow" @submitted="hideModalAdd" />
    </a-modal>

    <a-modal
      :maskClosable="false"
      v-if="modalEditShow"
      title="Sửa thông tin ngôn ngữ"
    ...
    >
      <LanguagesFormEdit v-if="modalEditShow" @submitted="hideModalEdit" :initialValues="editData" />
    </a-modal>
  </div>
</template>
...
import LanguagesFormAdd from '~/components/Languages/LanguagesFormAdd';
import LanguagesFormEdit from '~/components/Languages/LanguagesFormEdit';
...
export default {
  ...
  methods: {
    ...
    async handleDelete(id) {
      ...
      try {
        const doDelete = await this.$store.dispatch('languages/delete', id);
        if (!doDelete.success && doDelete.err_msg) {
          ...
          this.$message.error('Xóa ngôn ngữ không thành công');
      }
