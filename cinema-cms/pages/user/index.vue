<template>
  <div class="agency-table-groups">
    ...
    <div class="text-right">
      <a-button type="primary" class="editable-add-btn mb-3" @click="showModalAdd">Thêm người dùng mới</a-button>
    </div>
    <a-table
      size="middle"
      :columns="columns"
    ...
    >
      ...
      <template slot="role" slot-scope="text, record">
        <ul style="padding-left: 18px;">
          <li v-for="(role, index) in text" :key="index">{{role.name}}</li>
        </ul>
      </template>

      <template slot="status" slot-scope="text, record">
        {{text == 0 ? "Không kích hoạt" : "Kích hoạt"}}
      </template>
      ...
      <template slot="operation" slot-scope="text, record">
        <div class="editable-row-operations" v-if="record.id != userId">
...
export default {
  ...
  async fetch({ store }) {
    await Promise.all([
      ...
      store.dispatch('roles/getOptions'),
    ]);
  },
    ...
    async handleDelete(id) {
      ...
      try {
        ...
        if (!doDelete.success && doDelete.err_msg) {
          ...
          this.$message.error('Xóa người dùng không thành công');
      }
