<template>
  <div class="agency-table-groups">
    <h2>Quản lý phòng chiếu phim</h2>
    <div class="text-right">
      <a-button type="primary" class="editable-add-btn mb-3" @click="showModalAdd">Thêm phòng chiếu phim mới</a-button>
    </div>
    <a-table
      size="middle"
      :columns="columns"
      :dataSource="movieRooms"
    ...
    <a-modal
      :maskClosable="false"
      title="Thêm phòng chiếu phim mới"
      ...
      @cancel="hideModalAdd"
    >
      <MovieRoomsFormAdd v-if="modalAddShow" @submitted="hideModalAdd" />
    </a-modal>

    <a-modal
      :maskClosable="false"
      v-if="modalEditShow"
      title="Sửa thông tin phòng chiếu phim"
    ...
    >
      <MovieRoomsFormEdit v-if="modalEditShow" @submitted="hideModalEdit" :initialValues="editData" />
    </a-modal>
  </div>
</template>
...
import MovieRoomsFormAdd from '~/components/MovieRooms/MovieRoomsFormAdd';
import MovieRoomsFormEdit from '~/components/MovieRooms/MovieRoomsFormEdit';
...
export default {
  ...
  async fetch({ store }) {
    await Promise.all([
      store.dispatch('movieRooms/get'),
      store.dispatch('cinemas/getOptions'),
      store.dispatch('rooms/getOptions'),
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          title: 'Rạp',
          dataIndex: 'room.cinemaDetail.name'
        },
        {
          title: 'Người tạo',
          dataIndex: 'createdBy',
  ...
  computed: {
    ...mapState('movieRooms', ['movieRooms']),
  ...
  methods: {
    ...
    async handleDelete(id) {
      ...
      try {
        const doDelete = await this.$store.dispatch('movieRooms/delete', id);
        if (!doDelete.success && doDelete.err_msg) {
          this.$message.error(doDelete.err_msg);
        } else {
          this.$message.success('Xóa phòng chiếu phim thành công!');
          await this.reloadTableData();
        }
      } catch (error) {
          this.$message.error('Xóa phòng chiếu phim không thành công');
      }
    ...
    async reloadTableData(reloadOption) {
      ...
      await this.$store.dispatch('movieRooms/get', {});
