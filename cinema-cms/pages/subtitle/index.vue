<template>
  <div class="agency-table-groups">
    <h2>Quản lý phụ đề</h2>
    <div class="text-right">
      <a-button type="primary" class="editable-add-btn mb-3" @click="showModalAdd">Thêm phụ đề mới</a-button>
    </div>
    <a-table
      size="middle"
      :columns="columns"
    ...
    <a-modal
      ...
      @cancel="hideModalAdd"
    >
      <SubtitlesFormAdd v-if="modalAddShow" @submitted="hideModalAdd" />
    </a-modal>

    <a-modal
      :maskClosable="false"
      v-if="modalEditShow"
      title="Sửa thông tin phụ đề"
    ...
    >
      <SubtitlesFormEdit v-if="modalEditShow" @submitted="hideModalEdit" :initialValues="editData" />
    </a-modal>
  </div>
</template>
...
import SubtitlesFormAdd from '~/components/Subtitles/SubtitlesFormAdd';
import SubtitlesFormEdit from '~/components/Subtitles/SubtitlesFormEdit';
...
export default {
  ...
  methods: {
    ...
    async handleDelete(id) {
      ...
      try {
        const doDelete = await this.$store.dispatch('subtitles/delete', id);
        if (!doDelete.success && doDelete.err_msg) {
          this.$message.error(doDelete.err_msg);
        } else {
          this.$message.success('Xóa phụ đề thành công!');
          await this.reloadTableData();
        }
      } catch (error) {
          this.$message.error('Xóa phụ đề không thành công');
      }
    ...
    async reloadTableData(reloadOption) {
      ...
      await this.$store.dispatch('subtitles/get', {});
