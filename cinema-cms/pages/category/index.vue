<template>
  <div class="agency-table-groups">
    ...
    <div class="text-right">
      <a-button type="primary" class="editable-add-btn mb-3" @click="showModalAdd">Thêm thể loại mới</a-button>
    </div>
    <a-table
      size="middle"
      :columns="columns"
    ...
    <a-modal
      ...
      @cancel="hideModalAdd"
    >
      <CategoriesFormAdd v-if="modalAddShow" @submitted="hideModalAdd" />
    </a-modal>
    ...
    >
      <CategoriesormEdit v-if="modalEditShow" @submitted="hideModalEdit" :initialValues="editData" />
    </a-modal>
  </div>
</template>
...
import CategoriesFormAdd from '~/components/Categories/CategoriesFormAdd';
import CategoriesormEdit from '~/components/Categories/CategoriesormEdit';
...
export default {
  ...
  methods: {
    ...
    async handleDelete(id) {
      ...
      try {
        const doDelete = await this.$store.dispatch('categories/delete', id);
        if (!doDelete.success && doDelete.err_msg) {
          ...
          this.$message.error('Xóa thể loại không thành công');
      }
