<template>
  <div class="agency-table-groups">
    <h2>Quản lý ca chiếu</h2>
    <div class="text-right">
      <a-button type="primary" class="editable-add-btn mb-3" @click="showModalAdd">Thêm ca chiếu mới</a-button>
    </div>
    <a-table
      size="middle"
      :columns="columns"
      :dataSource="movieShifts"
    ...
    <a-modal
      ...
      @cancel="hideModalAdd"
    >
      <MovieShiftsFormAdd v-if="modalAddShow" @submitted="hideModalAdd" />
    </a-modal>

    <a-modal
      :maskClosable="false"
      v-if="modalEditShow"
      title="Sửa thông tin ca chiếu"
    ...
    >
      <MovieShiftsFormEdit v-if="modalEditShow" @submitted="hideModalEdit" :initialValues="editData" />
    </a-modal>
  </div>
</template>
...
import MovieShiftsFormAdd from '~/components/MovieShifts/MovieShiftsFormAdd';
import MovieShiftsFormEdit from '~/components/MovieShifts/MovieShiftsFormEdit';
...
export default {
  ...
  async fetch({ store }) {
    await Promise.all([
      store.dispatch('movieShifts/get'),
  ...
  computed: {
    ...mapState('movieShifts', ['movieShifts']),
  ...
  methods: {
    ...
    async handleDelete(id) {
      ...
      try {
        const doDelete = await this.$store.dispatch('movieShifts/delete', id);
        if (!doDelete.success && doDelete.err_msg) {
          this.$message.error(doDelete.err_msg);
        } else {
          this.$message.success('Xóa ca chiếu thành công!');
          await this.reloadTableData();
        }
      } catch (error) {
          this.$message.error('Xóa ca chiếu không thành công');
      }
    ...
    async reloadTableData(reloadOption) {
      ...
      await this.$store.dispatch('movieShifts/get', {});
