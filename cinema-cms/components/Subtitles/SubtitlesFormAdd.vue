          ...
          <a-input
            ...
            v-decorator="['subtitle', {
                rules: [
                  ...
                  { pattern: /^[ a-zA-Z0-9_ÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêếìíòóôõùúăđĩũơƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼỀỀỂưăạảấầẩẫậắằẳẵặẹẻẽềềểỄỆỈỊỌỎỐỒỔỖỘỚỜỞỠỢỤỦỨỪễệỉịọỏốồổỗộớờởỡợụủứừỬỮỰỲỴÝỶỸửữựỳỵỷỹ]+$/g, message: 'Không được chứa ký tự đặc biệt'}
                ]
              }]"
          />
        </a-form-item>
    ...
    submit(e) {
      ...
      this.form.validateFields(async (error, values) => {
        ...
        if (!error) {
          ...
          const doAdd = await this.$store.dispatch("subtitles/add", {
            ...values,
          });
          ...
          if (doAdd.errors || doAdd.message) {
            ...
            this.$message.success("Thêm phụ đề thành công!");
            this.$emit("submitted");
          }
        } else {
