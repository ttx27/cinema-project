<template>
  <a-form :form="form">
    <a-row :gutter="24">
      <a-col :span="24">
        <a-form-item label="Ca chiếu (tiếng)">
          <a-input-number
            style="width: 100%"
            placeholder="Nhập ca chiếu"
            v-decorator="['time', {
              rules: [
    ...
    submit(e) {
      ...
      this.form.validateFields(async (error, values) => {
        if (!error) {
          ...
          const doEdit = await this.$store.dispatch('movieShifts/edit', {
            ...values,
          ...
          if (doEdit.errors || doEdit.message) {
            ...
            this.$message.success("Cập nhật thông tin ca chiếu thành công!");
            this.$emit("submitted");
          }
        } else {
