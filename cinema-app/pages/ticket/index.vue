  ...
  computed: {
    ...
    seatList() {
      ...
      return this.seats ? sortBy(sortBy(this.seats, 'name'), 'number')
        ...
        .map(seat => {
          const isEmpty = this.billDetails.filter(item => item.seat && item.seat.id == seat.id && item.bill && item.bill.showTimeDetail && item.bill.showTimeDetail.id == this.showTime.id).length == 0;
